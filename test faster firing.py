...
def play_tone(f, volume=0.5, fs=44100, duration=0.01):
    """
    Plays a tone with the specified frequency, volume, and duration.
...
if __name__ == "__main__":
    ...
    initial_volume = 0.5  # Initial volume for sounds
    ...
    while time.time() - start_time < total_duration:
        for index in firing_order:
            play_tone(combustion_frequencies[index - 1], current_volume)

            # Simulate overlaying by adjusting volume (not ideal)
            current_volume = initial_volume * 0.5  # Reduce volume slightly
            play_tone(exhaust_frequencies[index - 1], current_volume)
            # Reset volume for next iteration
            current_volume = initial_volume